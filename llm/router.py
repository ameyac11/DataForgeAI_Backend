...
GITHUB_MODELS = {"gpt-4.1", "gpt-4o-mini"}
...
def generate_text(messages: list, model_id: str = None, temperature: float = 0.5, max_tokens: int = 8192) -> str:
    ...
    for mid in models_to_try:
        ...
        try:
            provider = get_provider(mid)
            result = provider.generate_completion(messages, mid, temperature=temperature, max_tokens=max_tokens)
            if result:
                return result
        except Exception:
            continue
